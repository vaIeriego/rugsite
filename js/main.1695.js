...
if (isRefreshLikeLoad) {
  ...
  if (
    ...
    !document.body.classList.contains("page-traditional")
  ) {
...
const initTraditionalCopyBlockWaterfallReveal = () => {
  ...
  if (bodyCopy) {
    bodyCopy.classList.remove("traditional-body-waterfall");
  }
  textBlock.classList.remove("fixed-rise-reveal", "is-revealed");
  textBlock.classList.add("scroll-reveal");
  textBlock.style.setProperty("--reveal-delay", "0ms");

  const revealTextBlock = () => {
    if (!textBlock.classList.contains("is-revealed")) {
      textBlock.classList.add("is-revealed");
    }
  };
  ...
  window.requestAnimationFrame(() => {
    window.requestAnimationFrame(revealTextBlock);
  });
};
