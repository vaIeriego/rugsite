...
const initFixedOneShotReveals = () => {
  ...
  if (document.body.classList.contains("page-traditional")) {
    const traditionalTextBlock = document.querySelector(
    ...
    if (traditionalTextBlock) targets.push(traditionalTextBlock);
    if (traditionalArrows) targets.push(traditionalArrows);
  }

  if (document.body.classList.contains("page-home")) {
    const floralWrap = document.querySelector(
      ".hero-square-carousel__collection-two-image-wrap"
    );
    if (floralWrap) {
      targets.push(floralWrap);
      const floralBlocks = floralWrap.querySelectorAll(".hero-square-carousel__floral-block");
      floralBlocks.forEach((block) => {
        const isRedOrPink =
          block.classList.contains("hero-square-carousel__floral-block--red") ||
          block.classList.contains("hero-square-carousel__floral-block--pink");
        if (isRedOrPink) {
          block.classList.remove("fixed-rise-reveal", "is-revealed");
          return;
        }
        targets.push(block);
      });
...
startTraditionalBodyCopyWaterfall();
  ...
  const render = () => {
    ...
    groupStates.forEach((group) => {
      ...
      group.members.forEach((member) => {
        ...
        member.node.style.setProperty(member.variableName, yValue);
      });
    });
...
const initHomeTextSectionParallax = () => {
  ...
  const configs = [
    { selector: ".hero-square-carousel__kees", speed: 0.152, max: 52 },
    { selector: ".hero-square-carousel__collection-three-text", speed: 0.16, max: 52 },
    { selector: ".hero-square-carousel__collection-four-text", speed: 0.16, max: 52 },
    { selector: ".hero-square-carousel__collection-five-text", speed: 0.16, max: 52 },
  ];
...
const initPostHeroScrollReveal = () => {
  ...
  const IMAGE_TO_TEXT_DELAY_MS = 480;
  const TRADITIONAL_TEXT_DELAY_MS = 1040;
  const SECTION_REVEAL_STAGGER_MS = 240;
  ...
  for (let block = heroSplit.nextElementSibling; block; block = block.nextElementSibling) {
    ...
    if (block.classList.contains("hero-follow-image")) {
      ...
      if (collectionsWrap) {
        ...
        collectionSections.forEach((section, sectionIndex) => {
          const isFloralSection = section.classList.contains("section-floral-reference");
          if (isFloralSection) {
            // Keep floral collage static, but still allow Botanical copy waterfall reveal.
            const floralTextPiece = section.querySelector(
              ".hero-square-carousel__collection-two-text"
            );
            if (floralTextPiece) {
              addRevealItem(
                floralTextPiece,
                sectionIndex * SECTION_REVEAL_STAGGER_MS,
                { noLift: true, requiresScroll: true }
              );
            }
            return;
          }
          const sectionDelayMs = sectionIndex * SECTION_REVEAL_STAGGER_MS;
          ...
          if (pieces.length) {
            ...
            if (imagePiece) {
              const isFloralImagePiece =
                imagePiece.classList.contains(
                  "hero-square-carousel__collection-two-image-wrap"
                );
              addRevealItem(imagePiece, sectionDelayMs, {
                noLift: isFloralImagePiece,
              });
            }
            if (textPiece) {
              addRevealItem(
                textPiece,
                sectionDelayMs + (imagePiece ? IMAGE_TO_TEXT_DELAY_MS : 0),
                { noLift: true }
              );
            }
            ...
            extraPieces.forEach((piece, idx) => {
              addRevealItem(
                piece,
                sectionDelayMs +
                  (imagePiece ? IMAGE_TO_TEXT_DELAY_MS : 0) +
...
const lockHomeStaticScrollTargets = () => {
  if (!document.body.classList.contains("page-home")) return;
  const targets = document.querySelectorAll(
    ".hero-square-carousel, .hero-square-carousel__collection-two, .hero-square-carousel__collection-two-image-wrap, .hero-square-carousel__collection-two-collage-overlay"
  );
  targets.forEach((node) => {
    node.classList.remove("scroll-reveal", "is-revealed", "scroll-reveal-no-lift");
    node.style.removeProperty("--reveal-delay");
